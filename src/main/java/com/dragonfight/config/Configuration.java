...
public class Configuration
{
    ...
    public void load()
    {
        final Path configPath = FabricLoader.getInstance().getConfigDir().normalize().resolve(DragonfightMod.MODID + ".json");
        final File config = configPath.toFile();

        if (!config.exists())
        {
            DragonfightMod.LOGGER.warn("Config not found, recreating default");
            save();
        }
        else
        {
            ...
            {
                DragonfightMod.LOGGER.error("Could not read config from:" + configPath, e);
            }
        }
    ...
    public void save()
    {
        ...
        {
            DragonfightMod.LOGGER.error("Could not write config to:" + configPath, e);
        }
    }
